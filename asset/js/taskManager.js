const createTaskHtml = (id, name, description, assignedTo, dueDate, status) => {
  ...
  if (status !== 'done') {
    markDoneBtn = `<button class="done-button" id="markDone" onclick="showDiv()">MarkDone</button>`
  }

  const html = `<li class="list-group-item taskDiv" data-task-id=${id}>
...
class TaskManager {
  ...
  load() {
    ...
    if (localStorage.getItem('currentId')) {
      // check localStorage for saved currentId
